...
import { PRODUCT_QUANTITY_MODAL_INPUT } from "../../utils/product-modal-utils.js";
...
function updateURL(option, value, add = true) {
  ...
  switch (option) {
    case 1:
      ...
      newURL = `${CURRENT_URL.origin}${CURRENT_URL.pathname}${
        URL_PARAMS.toString() === "" ? "" : `?${URL_PARAMS}`
      }`;
...
function preparePage() {
  ...
  if (categoryInfo.name == TOPICS.ANIME)
    efectoLoli(categoryInfo.bannerBackgroundName, categoryInfo.bannerTextName);

  //Ya que existe seteamos el banner
  setBanner(
    categoryInfo.bannerBackgroundName,
    categoryInfo.bannerTextName,
    categoryInfo.customStyleClassName
  );
...
function loadActiveFilters() {
  ...
  document.querySelectorAll(".filter-option").forEach((filter) => {
    ...
    if (
      ...
      filtersURL.some((el) => el.includes(filter.value))
    ) {
      filter.checked = true;

      document
        .getElementById(`filter-active-${filter.value}`)
        .classList.add("show-filter");
    ...
    filter.addEventListener("change", (e) => {
      ...
      document
        .getElementById(`filter-active-${filter.value}`)
        .classList.toggle("show-filter", filter.checked);
...
function hideActiveFilter() {
  ...
  document
    .querySelectorAll(".discart-filter-button")
...
function addRemoveFilterOption() {
  const deleteFilterElement = FILTERS_ACTIVE_CONTAINER.querySelector(
  ...
  const filtersActive =
    FILTERS_ACTIVE_CONTAINER.querySelectorAll(".show-filter");
...
function deleteAllFilters() {
  ...
  const discartButton = FILTERS_ACTIVE_CONTAINER.querySelector(
    "#discart-filter-all-button"
  );
  ...
  if (discartButton) {
    ...
    discartButton.addEventListener("click", () => {
      ...
      FILTERS_ACTIVE_CONTAINER.querySelectorAll(".show-filter").forEach(
        (filter) => {
...
document.addEventListener("click", function (event) {
  ...
  // Verifica si el clic fue fuera del botón y del panel
  if (
    !orderButton.contains(event.target) &&
    !orderPanel.contains(event.target) &&
